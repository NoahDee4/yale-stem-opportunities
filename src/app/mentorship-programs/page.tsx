...
import { MentorshipProgram, FieldTag, FIELD_TAGS } from "@/lib/types";
...
export default function MentorshipProgramsPage() {
  ...
  useEffect(() => {
    const q = query(collection(db, "mentorshipPrograms"), orderBy("datePosted", "desc"));
  ...
  const handleToggleFavorite = async (id: string) => {
    if (!user) { toast.error("Sign in to save programs"); return; }
    const ref = doc(db, "users", user.uid, "savedMentorshipPrograms", id);
    try {
  ...
  const filtered = useMemo(() => {
    return programs.filter((p) => {
      if (selectedFields.length > 0 && !p.fieldTags.some((f) => selectedFields.includes(f))) return false;
      if (searchQuery) {
  ...
  }, [programs, selectedFields, searchQuery]);

  const activeFilters = selectedFields.length;
    ...
    <div className="flex min-h-screen flex-col bg-white dark:bg-surface-dark">
      ...
      <main className="mx-auto w-full max-w-7xl flex-1 px-6 lg:px-10">
        <div className="pb-2 pt-12">
          ...
          >
            Curated links to established mentorship programs — shared by the Yale community
          </motion.p>
        </div>
        ...
        >
          <div className="flex gap-2">
            ...
            {user && (
              <Link href="/submit-mentorship-program" className="btn-primary hidden !py-2.5 md:inline-flex">
        ...
        <div className="mt-6 mb-4">
          <p className="text-[13px] text-text-tertiary dark:text-text-dark-tertiary">
            {loading ? "Loading..." : (
              <><span className="font-semibold text-text-primary dark:text-text-dark-primary">{filtered.length}</span>{" "}{filtered.length === 1 ? "program" : "programs"}</>
            )}
          </p>
        </div>
        ...
        {loading ? (
          ...
          >
            ...
            <h3 className="mb-1 text-[15px] font-semibold text-text-primary dark:text-text-dark-primary">No programs found</h3>
      ...
      {user && (
        <Link
          href="/submit-mentorship-program"
          className="fixed bottom-6 right-6 z-50 flex h-14 w-14 items-center justify-center rounded-full bg-black shadow-lg transition-transform hover:scale-105 dark:bg-white md:hidden"
        >
          <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="dark:stroke-black">
            <line x1="12" x2="12" y1="5" y2="19" /><line x1="5" x2="19" y1="12" y2="12" />
          </svg>
        </Link>
      )}
