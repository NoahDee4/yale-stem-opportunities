...
import TagFlashcards from "@/components/TagFlashcards";
...
export default function HomePage() {
  ...
  useEffect(() => {
    const q = query(collection(db, "opportunities"), orderBy("datePosted", "desc"));
    const unsubscribe = onSnapshot(q, (snapshot) => {
      const opps: Opportunity[] = snapshot.docs.map((doc) => {
        const data = doc.data();
        return {
          id: doc.id,
          title: data.title,
    ...
    <div className="flex min-h-screen flex-col bg-white dark:bg-surface-dark">
      ...
      <main className="mx-auto w-full max-w-7xl flex-1 px-6 lg:px-10">
        ...
        <div className="pb-2 pt-12">
          ...
          >
            Internships, research, fellowships & workshops from the Yale community
          </motion.p>
        </div>
        ...
        >
          <div className="flex gap-2">
            ...
            <div className="relative flex-1">
              <svg
                className="absolute left-3.5 top-1/2 -translate-y-1/2 text-text-tertiary dark:text-text-dark-tertiary"
            ...
            {/* Post button (if signed in, desktop) */}
            {user && (
              <Link href="/submit" className="btn-primary hidden !py-2.5 md:inline-flex">
          ...
          {activeFilters > 0 && !showFilters && (
            <div className="mt-3 flex flex-wrap items-center gap-1.5">
              ...
              {selectedFields.map((t) => (
                <button
                  key={t}
                  onClick={() => setSelectedFields(selectedFields.filter((x) => x !== t))}
                  className={`flex items-center gap-1 rounded-lg px-2.5 py-1 text-[11px] font-semibold transition-colors ${getTagColor(t).pill}`}
                >
                  {t}
                  <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M18 6L6 18M6 6l12 12" /></svg>
        ...
        <div className="mt-6 mb-4 flex items-center justify-between">
          <p className="text-[13px] text-text-tertiary dark:text-text-dark-tertiary">
            {loading ? (
              ...
              <>
                <span className="font-semibold text-text-primary dark:text-text-dark-primary">{filtered.length}</span>{" "}
            ...
            {[...Array(8)].map((_, i) => (
              <div key={i} className="animate-pulse rounded-2xl border border-border p-5 dark:border-border-dark">
                ...
                <div className="flex gap-1.5">
                  ...
                  <div className="h-5 w-12 rounded-md bg-surface-tertiary dark:bg-surface-dark-tertiary" />
                </div>
              </div>
            ))}
          </div>
